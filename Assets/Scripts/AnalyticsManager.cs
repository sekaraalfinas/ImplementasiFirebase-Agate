...
public static class AnalyticsManager
{
    ...
    public static void LogUpgradeEvent (int resourceIndex, int level)
    {
        ...
        LogEvent(
            FirebaseAnalytics.EventLevelUp, new Parameter(FirebaseAnalytics.ParameterIndex, resourceIndex.ToString()),
            new Parameter(FirebaseAnalytics.ParameterLevel, level)
            );

        // Karena resourceIndex digunakan sebagai ID, maka seharusnya kita menyimpannya sebagai string bukan int
    }
